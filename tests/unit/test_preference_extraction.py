...
@pytest.fixture()
def empty_profile() -> CustomerProfile:
    return CustomerProfile(session_id="s1", tenant_id="t1")
...
def test_explicit_allergy_adds_to_hard_stops(extractor, empty_profile):
    ...
    assert "nuts" in profile.hard_stops
...
def test_cant_eat_pattern(extractor, empty_profile):
    ...
    assert "dairy" in profile.hard_stops
...
def test_vegan_adds_multiple_hard_stops(extractor, empty_profile):
    ...
    assert "dairy" in profile.hard_stops
    assert "eggs" in profile.hard_stops
...
def test_vegetarian_adds_meat_fish(extractor, empty_profile):
    ...
    assert "meat" in profile.hard_stops
    assert "fish" in profile.hard_stops


def test_hard_stops_deduplicated(extractor, empty_profile):
    ...
    assert p2.hard_stops.count("nuts") == 1


def test_multiple_constraints_in_one_message(extractor, empty_profile):
    profile = run(extractor.update_from_message(
        empty_profile, "I can't eat gluten or dairy", "user"
    ))
    assert "gluten" in profile.hard_stops
    assert "dairy" in profile.hard_stops
...
def test_openness_score_increases_for_bold_taste(extractor, empty_profile):
    profile = run(extractor.update_from_message(empty_profile, "I'm an adventurous eater", "user"))
    assert profile.openness_score > empty_profile.openness_score


def test_openness_score_decreases_for_safe_preference(extractor, empty_profile):
    profile = run(extractor.update_from_message(empty_profile, "I prefer safe, plain food", "user"))
    assert profile.openness_score < empty_profile.openness_score


def test_openness_score_clamped_at_1(extractor, empty_profile):
    p = empty_profile.model_copy(update={"openness_score": 0.95})
    for _ in range(10):
        p = run(extractor.update_from_message(p, "I love bold exotic food", "user"))
    assert p.openness_score <= 1.0


def test_openness_score_clamped_at_0(extractor, empty_profile):
    p = empty_profile.model_copy(update={"openness_score": 0.05})
    for _ in range(10):
        p = run(extractor.update_from_message(p, "I prefer plain safe food", "user"))
    assert p.openness_score >= 0.0
...
def test_profile_accumulates_across_messages(extractor, empty_profile):
    ...
    assert "shellfish" in p2.hard_stops
